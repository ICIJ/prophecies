...
from prophecies.core.models.task_record_review import StatusType as ReviewStatusType
...
class TestTask(TestCase):
    ...
    def test_progress_by_round_for_django(self):
        ...
        record_foo.reviews.add(TaskRecordReview.objects.create(round=1, status=ReviewStatusType.PENDING, checker=self.django))
        record_foo.reviews.add(TaskRecordReview.objects.create(round=1, status=ReviewStatusType.DONE, checker=self.olivia))
        # Round 2 is completed at 100% by django
        record_bar.reviews.add(TaskRecordReview.objects.create(round=2, status=ReviewStatusType.DONE, checker=self.django))
    ...
    def test_progress_by_round_on_two_tasks(self):
        ...
        record_painting.reviews.add(
            TaskRecordReview.objects.create(round=1, status=ReviewStatusType.DONE, checker=self.olivia))
        record_gallery.reviews.add(
            TaskRecordReview.objects.create(round=1, status=ReviewStatusType.DONE, checker=self.olivia))
        # Round 2 is completed at 0% by django on "Art" and 100% on "Shop"
        record_painting.reviews.add(
            TaskRecordReview.objects.create(round=2, status=ReviewStatusType.PENDING, checker=self.django))
        record_gallery.reviews.add(
            TaskRecordReview.objects.create(round=2, status=ReviewStatusType.DONE, checker=self.django))
    ...
    def test_progress_is_0(self):
        record1 = TaskRecord.objects.create(task=self.art, status=StatusType.UNASSIGNED)
        record2 = TaskRecord.objects.create(task=self.art, status=StatusType.UNASSIGNED)

        record1.reviews.add(
            TaskRecordReview.objects.create(round=1, status=ReviewStatusType.PENDING, checker=self.olivia))
        record1.reviews.add(
            TaskRecordReview.objects.create(round=2, status=ReviewStatusType.PENDING, checker=self.django))
        record2.reviews.add(
            TaskRecordReview.objects.create(round=1, status=ReviewStatusType.PENDING, checker=self.olivia))
    ...
    def test_progress_is_50(self):
        ...
        record2.reviews.add(
            TaskRecordReview.objects.create(round=1, status=ReviewStatusType.DONE, checker=self.olivia))
        record2.reviews.add(
            TaskRecordReview.objects.create(round=1, status=ReviewStatusType.PENDING, checker=self.django))
    ...
    def test_progress_is_100(self):
        ...
        record2.reviews.add(
            TaskRecordReview.objects.create(round=1, status=ReviewStatusType.DONE, checker=self.olivia))
        record2.reviews.add(
            TaskRecordReview.objects.create(round=1, status=ReviewStatusType.DONE, checker=self.django))
    ...
    def test_progress_is_100_without_mixing_tasks(self):
        ...
        TaskRecord.objects.create(task=self.art, status=StatusType.DONE)
        TaskRecord.objects.create(task=self.shop, status=StatusType.UNASSIGNED)
        self.assertEqual(self.art.progress, 100)
