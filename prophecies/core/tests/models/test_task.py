...
class TestTask(TestCase):
    def setUp(self):
        ...
        self.art = Task.objects.create(
            name="Art", project=self.pencil_papers, color="#fe6565", rounds=3
        )
        self.shop = Task.objects.create(
            name="Shop", project=self.pencil_papers, rounds=3
        )
    ...
    def test_progress_by_round_for_django(self):
        ...
        record_foo.reviews.add(
            TaskRecordReview.objects.create(
                round=1, status=ReviewStatusType.PENDING, checker=self.django
            )
        )
        record_foo.reviews.add(
            TaskRecordReview.objects.create(
                round=1, status=ReviewStatusType.DONE, checker=self.olivia
            )
        )
        # Round 2 is completed at 100% by django
        record_bar.reviews.add(
            TaskRecordReview.objects.create(
                round=2, status=ReviewStatusType.DONE, checker=self.django
            )
        )
    ...
    def test_progress_by_round_on_two_tasks(self):
        ...
        record_painting.reviews.add(
            TaskRecordReview.objects.create(
                round=2, status=ReviewStatusType.PENDING, checker=self.django
            )
        )
        record_gallery.reviews.add(
            TaskRecordReview.objects.create(
    ...
    def test_progress_is_0(self):
        ...
        record1.reviews.add(
            TaskRecordReview.objects.create(
                round=1, status=ReviewStatusType.PENDING, checker=self.olivia
            )
        )
        record1.reviews.add(
            TaskRecordReview.objects.create(
    ...
    def test_progress_is_100(self):
        ...
        record2.reviews.add(
            TaskRecordReview.objects.create(
                round=1, status=ReviewStatusType.DONE, checker=self.django
            )
        )
    ...
    def test_it_opens_the_task(self):
        task = Task.objects.create(
            name="Foo", status="CLOSED", project=self.pencil_papers
        )
        self.assertEqual(task.status, "CLOSED")
        task.open()
        self.assertEqual(task.status, "OPEN")
    ...
    def test_it_log_action_when_task_is_locked(self):
        task = Task.objects.create(
            name="Foo", project=self.pencil_papers, creator=self.olivia
        )
        task.lock()
    ...
    def test_it_log_action_when_task_is_open(self):
        task = Task.objects.create(
            name="Foo", project=self.pencil_papers, creator=self.olivia, status="CLOSED"
        )
        task.open()
        action = Action.objects.filter_actor(actor=self.olivia).first()
        self.assertTrue(action is not None)
