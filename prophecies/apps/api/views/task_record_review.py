...
class TaskRecordReviewSerializer(serializers.HyperlinkedModelSerializer):
    checker = ResourceRelatedField(many=False, read_only=True)
    choice = ResourceRelatedField(
        many=False, queryset=Choice.objects, required=False, allow_null=True
    )
    task_record = ResourceRelatedField(many=False, read_only=True)
    task_id = serializers.CharField(read_only=True)
    history = SerializerMethodResourceRelatedField(
        many=True, model=TaskRecordReview, read_only=True
    )
    included_serializers = {
        "checker": UserSerializer,
        "choice": ChoiceSerializer,
        "task_record": TaskRecordSerializer,
        "task_record__medias": TaskRecordMediaSerializer,
    ...
    def filter_choice_queryset(
        self, f, instance=None, data=serializers.empty, request=None, **kwargs
    ):
        ...
        if "choice" in self.validated_data:
            ...
            if has_choice:
                action.send(
                    ...
                    action_object=self.validated_data.get("choice"),
    ...
    def get_root_meta(self, resource, many):
        if many:
            ...
            return {"countBy": list(count_by_task) + list(count_by_choice)}
        return {}
...
class TaskRecordReviewViewSet(views.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return (
            TaskRecordReview.objects.user_scope(self.request.user)
            ...
            .prefetch_related("task_record__medias")
        )
    ...
    def check_object_permissions(self, request, obj):
        ...
        if "choice" in request.data and not obj.task_record.task.is_open:
            raise exceptions.PermissionDenied(
                detail="This resource's task was locked by a user."
            )
        if "choice" in request.data and obj.task_record.locked:
            raise exceptions.PermissionDenied(
                detail="This resource was locked by a user."
            )
