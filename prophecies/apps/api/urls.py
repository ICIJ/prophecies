...
from prophecies.apps.api.views.task_user_choice_statistics import (
    TaskUserChoiceStatisticsViewSet,
)
...
router.register(
    r"action-aggregates", ActionAggregateViewSet, basename="action-aggregates"
)
router.register(
    r"task-user-choice-statistics",
    TaskUserChoiceStatisticsViewSet,
    basename="task-user-choice-statistics",
)
router.register(
    r"task-user-statistics", TaskUserStatisticsViewSet, basename="task-user-statistics"
)
...
urlpatterns = [
    ...
    path(
        ...
        view=TaskRecordViewSet.as_view({"get": "actions"}),
        name="task-record-actions",
    ),
    path(
        "v1/users/<pk>/relationships/actions/",
        view=UserViewSet.as_view({"get": "actions"}),
        name="user-actions",
    ),
]
