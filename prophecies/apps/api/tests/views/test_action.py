...
class TestAction(TestCase):
    ...
    def test_list_returns_actions_with_verb_attribute(self):
        ...
        request = self.client.get('/api/v1/actions/')
        self.assertEqual(request.status_code, 200)
        first_action = request.json().get('data')[0]
        self.assertEqual(first_action.get('attributes').get('verb'), 'added')
    ...
    def test_list_returns_actions_with_actor_relationship(self):
        ...
        self.assertEqual(request.status_code, 200)
        first_action = request.json().get('data')[0]
        self.assertIn('actor', first_action.get('relationships', {}))
    ...
    def test_list_returns_actions_with_target_relationship(self):
        ...
        self.assertEqual(request.status_code, 200)
        first_action = request.json().get('data')[0]
        self.assertIn('target', first_action.get('relationships', {}))
    ...
    def test_list_returns_actions_including_actor_relationship(self):
        ...
        request = self.client.get('/api/v1/actions/')
        self.assertEqual(request.status_code, 200)
        first_included = request.json().get('included')[0]
        self.assertEqual(first_included.get('type'), 'User')
        self.assertEqual(first_included.get('id'), '1')
        self.assertEqual(first_included.get('attributes').get('username'), 'olivia')
    ...
    def test_list_return_only_olivia_actions(self):
        ...
        self.client.login(username='olivia', password='olivia')
        request = self.client.get(f'/api/v1/actions/?filter[user_stream]={self.olivia.id}')
        action_list = request.json().get('data')
        self.assertEqual(len(action_list), 2)
        request = self.client.get(f'/api/v1/actions/?filter[user_stream]={self.ruby.id}')
        action_list = request.json().get('data')
