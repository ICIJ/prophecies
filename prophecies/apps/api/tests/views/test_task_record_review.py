...
class TestTaskRecordReview(TestCase):
    ...
    def setUp(self):
        ...
        self.task = Task.objects.create(
            name="paintings", project=project, choice_group=choice_group
        )
        self.task.checkers.add(self.olivia)
        self.task.checkers.add(self.django)
        # Add a series of records
        self.task_record_foo = TaskRecord.objects.create(
            original_value="foo", task=self.task
        )
        self.task_record_bar = TaskRecord.objects.create(
            original_value="bar", task=self.task
        )
        self.task_record_baz = TaskRecord.objects.create(
            original_value="baz", task=self.task
        )
        self.task_record_qux = TaskRecord.objects.create(
            original_value="qux", task=self.task
        )
    ...
    def test_it_returns_two_task_record_reviews_for_olivia(self):
        TaskRecordReview.objects.create(
            task_record=self.task_record_foo, checker=self.olivia
        )
        TaskRecordReview.objects.create(
            task_record=self.task_record_bar, checker=self.olivia
        )
    ...
    def test_it_returns_none_task_record_reviews_for_user_not_in_any_tasks(self):
        ...
        TaskRecordReview.objects.create(
            task_record=self.task_record_bar, checker=self.django
        )
    ...
    def test_it_returns_one_task_record_review_for_django(self):
        TaskRecordReview.objects.create(
            task_record=self.task_record_foo, checker=self.django
        )
    ...
    def test_it_returns_django_task_record_review(self):
        attribution = TaskRecordReview.objects.create(
            task_record=self.task_record_foo, checker=self.django
        )
        self.client.login(username="django", password="django")
        ...
        self.assertEqual(
            relationships["taskRecord"]["data"]["id"], str(self.task_record_foo.id)
        )
    ...
    def test_it_returns_task_record_with_link_from_task(self):
        ...
        task_record_entity = next(
            entity
            for entity in included
            if entity["type"] == "TaskRecord"
            and entity["id"] == str(self.task_record_foo.id)
        )
        self.assertEqual(
            task_record_entity["attributes"]["link"], "https://icij.org/foo.json"
        )
    ...
    def test_it_returns_task_record_with_choice_id(self):
        ...
        attribution = TaskRecordReview.objects.create(
            task_record=self.task_record_foo, checker=self.django, choice=choice
        )
        self.client.login(username="django", password="django")
        request = self.client.get(f"/api/v1/task-record-reviews/{attribution.id}/")
        data = request.json().get("data")
        self.assertEqual(
            data.get("relationships").get("choice").get("data").get("id"),
            str(choice.id),
        )
    ...
    def test_it_returns_number_of_task_record_reviews_for_olivia_by_task(self):
        ...
        request = self.client.get("/api/v1/task-record-reviews/")
        self.assertEqual(request.status_code, 200)
        count_by = request.json().get("meta").get("countBy")
    ...
    def test_it_cannot_set_task_record_choice_with_normal_user(self):
        ...
        request = self.client.put(
            f"/api/v1/task-record-reviews/{attribution.id}/",
            payload,
    ...
    def test_it_can_make_changes_to_note_on_locked_record(self):
        task_record_with_note = TaskRecord.objects.create(
            original_value="foo", task=self.task
        )
        task_record_with_note.locked = True
        task_record_with_note.save()
        attribution = TaskRecordReview.objects.create(
            task_record=task_record_with_note,
            checker=self.django,
            note="this is a note",
        )
        self.client.login(username="django", password="django")
        payload = {
            "data": {
                "type": "TaskRecordReview",
                "id": attribution.id,
                "attributes": {"note": "this is an edited note"},
            }
        }
        request = self.client.put(
            f"/api/v1/task-record-reviews/{attribution.id}/",
    ...
    def test_it_returns_two_review_when_bookmarked_one_record(self):
        TaskRecordReview.objects.create(
            task_record=self.task_record_foo, checker=self.django, round=1
        )
        TaskRecordReview.objects.create(
            task_record=self.task_record_foo, checker=self.django, round=2
        )
        self.task_record_foo.bookmarked_by.add(self.django)

        self.client.login(username="django", password="django")
        request = self.client.get(
            f"/api/v1/task-record-reviews/?filter[task_record__bookmarked_by]={self.django.id}"
        )
        data = request.json().get("data")
        self.assertEqual(len(data), 2)
    ...
    def test_it_returns_bookmarked_reviews_for_an_user(self):
        ...
        TaskRecordReview.objects.create(
            task_record=self.task_record_foo, checker=self.olivia, round=2
        )
        TaskRecordReview.objects.create(
            task_record=self.task_record_bar, checker=self.django, round=1
        )
        TaskRecordReview.objects.create(
            task_record=self.task_record_baz, checker=self.django, round=1
        )
        self.task_record_foo.bookmarked_by.add(self.django)
        ...
        self.client.login(username="django", password="django")
        request = self.client.get(
            f"/api/v1/task-record-reviews/?filter[checker]="
            f"{self.django.id}&filter[task_record__bookmarked_by]={self.django.id}"
        )
        data = request.json().get("data")
        self.assertEqual(len(data), 2)
        ...
        request = self.client.get(
            f"/api/v1/task-record-reviews/?filter[checker]="
            f"{self.olivia.id}&filter[task_record__bookmarked_by]={self.olivia.id}"
        )
        data = request.json().get("data")
        self.assertEqual(len(data), 1)
    ...
    def test_it_returns_bookmarked_reviews_for_multiple_users(self):
        ...
        self.client.login(username="django", password="django")
        request = self.client.get(
            f"/api/v1/task-record-reviews/?"
            f"filter[checker]=2&filter[task_record__bookmarked_by]={self.olivia.id},{self.django.id}"
        )
        data = request.json().get("data")
    ...
    def test_it_returns_reviews_from_records_with_all_rounds_reviewed(self):
        ...
        self.client.login(username="django", password="django")
        request = self.client.get(
            "/api/v1/task-record-reviews/?filter[task_record__all_rounds_reviewed]=1"
        )
        data = request.json().get("data")
        self.assertEqual(len(data), 3)

        request = self.client.get(
            "/api/v1/task-record-reviews/?filter[task_record__all_rounds_reviewed]=0"
        )
        data = request.json().get("data")
        self.assertEqual(len(data), 1)

        request = self.client.get(
            "/api/v1/task-record-reviews/?filter[task_record__all_rounds_reviewed]=null"
        )
        data = request.json().get("data")
        self.assertEqual(len(data), 4)

    def test_it_check_stat_deletion_on_task_record_review_delete(self):
        self.task.choice_group.choices.first()
        trr = TaskRecordReview.objects.create(
            task_record=self.task_record_foo, checker=self.django, round=1
        )
