<template>
  <div class="stats-by-users d-flex flex-column flex-grow-1 py-3">
    <div v-for="(user, index) in users" :key="index" class="stats-by-users__row d-flex flex-row py-2">
      ...
      <div class="stats-by-users__row__done" :class="tableClassList" :title="$t('taskStatsCard.doneRecords')">
        <span class="stats-by-users__row__number">{{ user.done }}</span
        ><check-icon size="1x" class="text-primary ml-2" />
      </div>
      <div class="stats-by-users__row__pending" :class="tableClassList" :title="$t('taskStatsCard.pendingRecords')">
        <span class="stats-by-users__row__number">{{ user.pending }}</span
        ><clock-icon size="1x" class="text-danger ml-2" />
      </div>
    </div>
    <div
      v-if="withTotal"
      class="stats-by-users__total d-flex flex-row py-3 font-weight-bold flex-grow-1 align-items-end"
    >
      <div class="col-3 pl-0">Total</div>
      ...
      <div :class="tableClassList" :title="$t('taskStatsCard.doneRecords')">
        <span class="stats-by-users__row__number">{{ totalStats.done }}</span
        ><check-icon size="1x" class="text-primary ml-2" />
      </div>
      <div :class="tableClassList" :title="$t('taskStatsCard.pendingRecords')">
        <span class="stats-by-users__row__number">{{ totalStats.pending }}</span
        ><clock-icon size="1x" class="text-danger ml-2" />
      </div>
    </div>
  </div>
