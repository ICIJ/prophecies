  ...
  <div class="stats-by-round d-flex flex-column" :class="{ 'stats-by-round--extended': extended }">
    <template v-if="extended">
      <div class="stats-by-round__progress d-flex py-3" :class="hasUsers ? 'text-primary' : 'text-secondary'">
        ...
        <div class="stats-by-round__progress__value col-9">
          ...
          <span v-else class="stats-by-round__progress__value--no-record">{{
            $t('taskStatsCard.noRecordsAssigned')
          }}</span>
        </div>
      </div>
      <template v-if="hasUsers">
        ...
        <div class="stats-by-round__summary row py-3">
          <div
            v-for="statChoice in choiceStats"
            :key="statChoice.value"
            class="stats-by-round__summary__choice d-flex align-items-center"
            :class="badgeColumnClass"
          >
            <choice-badge
    ...
    <template v-else>
      <span class="stats-by-round__progress">
        ...
        <span v-if="isNotLastRound(round)" class="text-secondary mx-2"> | </span>
      </span>
    </template>
  </div>
</template>
