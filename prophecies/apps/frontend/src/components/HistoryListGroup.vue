...
export default {
  ...
  computed: {
    ...
    items() {
      return [...this.mentions, ...this.closedTasks, ...this.tips, ...this.reviewedItems].map((item, i) => {
  ...
  methods: {
    ...
    countCheckedReviewsByDateUserIdTaskId(acc, checkedItem) {
      // compute checked reviews (reviewed - cancelled)
...
<template>
  <div v-if="visibleItems.length" class="history-list-group px-0 history-list-group" :class="isFluidClass">
    ...
    <slot name="footer">
      <div v-if="hasMoreToSee" class="history-list-group__see-more d-flex justify-content-center py-3">
        <button
          class="history-list-group__see-more__button btn btn-primary border font-weight-bold text-white"
          @click="loadMore"
        >
