...
<template>
  <div class="task-stats-card__status d-flex flex-column justify-content-between text-right"
    ...
    <div class='d-flex flex-column flex-grow-1 justify-content-between '>
      <div class="task-stats-card__status__top " :class="{'ml-5 ' : extended}">
        <template v-if="task.close">
          ...
          <template v-if="extended && celebrate"><span class="ml-2">🎉</span><span class="sr-only">{{taskLabel}}</span>
          </template>
        </template>
        <span v-else class="task-stats-card__status__top__priority rounded py-1 px-2 text-nowrap"
    ...
    <div v-if="extended" class="task-stats-card__read-tips pt-2 ">
      <router-link :to="{ name: 'tip-list', query: { 'filter[task]': task.id } }" class="btn btn-danger px-3"> Read
        tips
      </router-link>
    </div>
  </div>
