...
export default {
  ...
  computed: {
    choiceGroup() {
      return ChoiceGroup.query().with('choices').find(this.choiceGroupId)
    },
    hasChoice() {
...
<template>
  <ul class="choice-group-buttons list-unstyled row" :class="{ 'choice-group-buttons--has-choice': hasChoice }">
    <li
