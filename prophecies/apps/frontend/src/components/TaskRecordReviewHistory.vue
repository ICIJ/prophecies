...
export default {
  ...
  computed: {
    taskRecordReview() {
      return TaskRecordReview.query().with('checker').with('choice').find(this.taskRecordReviewId)
    },
    taskRecordId() {
...
<template>
  <div class="task-record-review-history">
    <div
      v-for="{ id, checker, alternativeValue, choice, note } in history"
      :key="id"
      class="task-record-review-history__checker d-flex p-1"
    >
      <div
        class="task-record-review-history__checker__name"
        :class="{ 'task-record-review-history__checker__name--is-me': checker.isMe }"
      >
        <user-link class="text-truncate" :user-id="checker.id">
          {{ checker.displayName }}
          <template v-if="checker.isMe"> ({{ $t('taskRecordReviewHistory.you') }}) </template>
        </user-link>
      </div>
      <div v-if="choice !== null" class="task-record-review-history__checker__choice">
        ...
        >
          <template v-if="checker.isMe" #afterTooltip>
        ...
        <template v-if="alternativeValue">
          ...
          <template v-else>
            <b-btn
              ...
              variant="link"
              class="p-0"
              :title="$t('taskRecordReviewHistory.useTheSameValue')"
              @click="selectSameChoice({ alternativeValue, choice })"
            >
              <span class="text-truncate text-dark px-2">
                {{ alternativeValue | alternativeValueName }}
              </span>
